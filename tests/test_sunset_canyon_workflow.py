...
from rok_assistant.tasks.sunset_canyon_workflow import (  # noqa: E402
...
def _opponent(slot: int, power: int, *, name: str = "", confidence: float = 0.94) -> SunsetCanyonOpponent:
    return SunsetCanyonOpponent(
        slot=slot,
...
class FakeSunsetCanyonDriver:
    def __init__(self) -> None:
        self.calls: list[str] = []
        self.open_campaign_result = ResourceGatheringActionResult(True, data={"scene": "CAMPAIGN"})
        self.open_sunset_result = ResourceGatheringActionResult(True, data={"scene": "SUNSET_CANYON"})
        self.scan = SunsetCanyonStateScan(
            SunsetCanyonScanStatus.READY,
            remaining_attempts=1,
            opponents=(
                _opponent(1, 2_000_000, name="High Power"),
                _opponent(2, 1_000_000, name="Low Power"),
            ),
            screenshot_path="runtime/screens/sunset-scan.png",
        )
        self.start_result = SunsetCanyonBattleStartResult(
            True,
            changed=True,
            screenshot_path="runtime/screens/battle-started.png",
        )
        self.skip_result = ResourceGatheringActionResult(True, data={"skipped": True})
        self.reports: list[SunsetCanyonBattleReport] = [
            SunsetCanyonBattleReport(
                SunsetCanyonBattleOutcome.VICTORY,
                handled=True,
    ...
    def open_sunset_canyon(self, _request, _character, _policy):
        self.calls.append("open_sunset_canyon")
        return self.open_sunset_result

    def inspect_sunset_canyon(self, _request, _character, _policy):
        self.calls.append("inspect_sunset_canyon")
        return self.scan
    ...
    def collect_battle_result(self, _request, _character, opponent, _policy):
        ...
        return SunsetCanyonBattleReport(
            SunsetCanyonBattleOutcome.VICTORY,
            handled=True,
            remaining_attempts=0,
        )
...
class SunsetCanyonWorkflowTest(unittest.TestCase):
    def setUp(self) -> None:
        self.temp_dir = tempfile.TemporaryDirectory()
        self.db = Database(Path(self.temp_dir.name) / "sunset-canyon.sqlite3")
        self.db.initialize()
        ...
        self.driver = FakeSunsetCanyonDriver()
    ...
    def _workflow(self) -> SunsetCanyonWorkflow:
        return SunsetCanyonWorkflow(
    ...
    def _request(
        ...
        *,
        ...
        run_key: str = "sunset-run",
        policy: SunsetCanyonPolicy | None = None,
    ) -> SunsetCanyonRequest:
        return SunsetCanyonRequest(
            instance_id=self.instance_id,
            instance_index=0,
            instance_name="MEmu 1",
            character_id=self.character_id,
            policy=policy or SunsetCanyonPolicy(),
            job_id=job_id,
            run_key=run_key,
        )

    def test_workflow_exposes_required_states_and_template_keys(self) -> None:
        self.assertEqual(SUNSET_CANYON_STATES, self._workflow().workflow_states)
        self.assertIn("campaign.sunset_canyon.entry", SUNSET_CANYON_TEMPLATE_KEYS)
        self.assertIn("sunset_canyon.result.victory", SUNSET_CANYON_TEMPLATE_KEYS)

    def test_no_attempts_remaining_skips_without_starting_battle(self) -> None:
        self.driver.scan = SunsetCanyonStateScan(
            SunsetCanyonScanStatus.NO_ATTEMPTS,
            remaining_attempts=0,
            message="Daily attempts exhausted.",
            screenshot_path="runtime/screens/no-attempts.png",
        )

        result = self._workflow().execute(self._request(run_key="sunset-none"))
    ...
    def test_victory_result_persists_opponent_result_and_remaining_attempts(self) -> None:
        job_id = self._job("sunset-victory")

        result = self._workflow().execute(
            self._request(job_id=job_id, run_key="sunset-victory")
        )
    ...
    def test_defeat_result_is_successful_completed_battle(self) -> None:
        self.driver.reports = [
            SunsetCanyonBattleReport(
                SunsetCanyonBattleOutcome.DEFEAT,
                handled=True,
                remaining_attempts=0,
        ...
        result = self._workflow().execute(self._request(run_key="sunset-defeat"))
    ...
    def test_result_popup_handling_failure_blocks_workflow(self) -> None:
        self.driver.reports = [
            SunsetCanyonBattleReport(
                SunsetCanyonBattleOutcome.UNKNOWN,
                handled=False,
                remaining_attempts=1,
        ...
        result = self._workflow().execute(self._request(run_key="sunset-popup-blocked"))
    ...
    def test_navigation_failure_blocks_before_attempt_scan(self) -> None:
        self.driver.open_sunset_result = ResourceGatheringActionResult(
            False,
            message="Campaign entry was not visible.",
            screenshot_path="runtime/screens/no-sunset.png",
        )

        result = self._workflow().execute(self._request(run_key="sunset-navigation"))

        self.assertEqual(WorkflowOutcome.BLOCKED, result.outcome)
        self.assertEqual("open_sunset_canyon", result.result["terminal_state"])
        self.assertEqual("Campaign entry was not visible.", result.result["failure_reason"])
        self.assertEqual(["open_campaign", "open_sunset_canyon", "open_sunset_canyon"], self.driver.calls)
        self.assertNotIn("inspect_sunset_canyon", self.driver.calls)
        self.assertFalse(any(call.startswith("start:") for call in self.driver.calls))

    def test_skip_battle_is_disabled_by_default(self) -> None:
        result = self._workflow().execute(self._request(run_key="sunset-no-skip"))
    ...
    def test_skip_battle_runs_only_when_policy_enables_it(self) -> None:
        policy = SunsetCanyonPolicy(allow_skip_battle=True)

        result = self._workflow().execute(self._request(run_key="sunset-skip", policy=policy))
    ...
    def test_policy_limit_is_respected_before_remaining_attempts(self) -> None:
        ...
        self.driver.reports = [
            SunsetCanyonBattleReport(SunsetCanyonBattleOutcome.VICTORY, handled=True, remaining_attempts=2),
            SunsetCanyonBattleReport(SunsetCanyonBattleOutcome.VICTORY, handled=True, remaining_attempts=1),
            SunsetCanyonBattleReport(SunsetCanyonBattleOutcome.VICTORY, handled=True, remaining_attempts=0),
        ]
        policy = SunsetCanyonPolicy(max_battles_per_run=2)

        result = self._workflow().execute(self._request(run_key="sunset-limit", policy=policy))
