...
ALLIANCE_RESOURCE_COLLECTION_WORKFLOW_KEY = "alliance-resource-collection"
ALLIANCE_RESOURCE_COLLECTION_TEMPLATE_KEYS = (
    "city.alliance.button",
    "alliance.menu.territory",
    "alliance.territory.resources_tab",
    "alliance.resources.panel",
    "alliance.resources.claimable_badge",
    "alliance.resources.claim_button",
    "alliance.resources.reward_overlay",
    "alliance.resources.no_claimable",
    "alliance.resources.verification_required",
)
...
class AllianceResourceScanStatus(StrEnum):
    READY = "READY"
...
def _resource_key(value: str) -> str:
    ...
    if not key:
        raise ValueError("Alliance resource key must be configured.")
    return key
...
class AllianceResourceCollectionPolicy:
    enabled_resource_keys: tuple[str, ...] = ("FOOD", "WOOD", "STONE", "GOLD")
    minimum_detector_confidence: float = 0.85
    max_claims_per_run: int = 12

    def normalized(self) -> AllianceResourceCollectionPolicy:
        enabled = tuple(dict.fromkeys(_resource_key(item) for item in self.enabled_resource_keys))
        if not enabled:
            raise ValueError("At least one alliance resource key must be enabled.")
        return AllianceResourceCollectionPolicy(
            enabled_resource_keys=enabled,
            minimum_detector_confidence=_require_confidence(
                self.minimum_detector_confidence,
                "minimum_detector_confidence",
            ),
            max_claims_per_run=_require_positive_int(self.max_claims_per_run, "max_claims_per_run"),
        )

    def to_json(self) -> dict[str, object]:
        normalized = self.normalized()
        return {
            "enabled_resource_keys": list(normalized.enabled_resource_keys),
            "minimum_detector_confidence": normalized.minimum_detector_confidence,
            "max_claims_per_run": normalized.max_claims_per_run,
        }


@dataclass(frozen=True)
class AllianceResourceCollectionRequest:
    instance_id: int
    instance_index: int
    instance_name: str
    character_id: int
    ...
    session_key: str = ""
    policy: AllianceResourceCollectionPolicy = field(default_factory=AllianceResourceCollectionPolicy)
...
@dataclass(frozen=True)
class AllianceResourceCollectionConfig:
...
@dataclass(frozen=True)
class AllianceResourceObservation:
    resource_key: str
    ...
    def normalized_resource_key(self) -> str:
        return _resource_key(self.resource_key)

    def to_json(self) -> dict[str, object]:
        return {
            "resource_key": self.normalized_resource_key(),
...
@dataclass(frozen=True)
class AllianceResourceScan:
    status: AllianceResourceScanStatus | str
    observations: tuple[AllianceResourceObservation, ...] = ()
    message: str = ""
    retryable: bool = False
    screenshot_path: str = ""
    data: dict[str, object] = field(default_factory=dict)

    def normalized_status(self) -> AllianceResourceScanStatus:
        if isinstance(self.status, AllianceResourceScanStatus):
            return self.status
        try:
            return AllianceResourceScanStatus(str(self.status).strip().upper())
        except ValueError as exc:
            valid = ", ".join(item.value for item in AllianceResourceScanStatus)
            raise ValueError(
                f"Invalid alliance resource scan status: {self.status!r}. Expected one of: {valid}."
            ) from exc
...
@dataclass(frozen=True)
class AllianceResourceClaimResult:
    success: bool
    changed: bool = False
    claimed_count: int = 0
    reward_overlay_present: bool = False
...
class AllianceResourceAccountPrecondition(Protocol):
    def ensure_account(
        self,
        request: AllianceResourceCollectionRequest,
        character: Character,
    ) -> ResourceGatheringActionResult:
        ...


class AllianceResourceCharacterPrecondition(Protocol):
...
class AllianceResourceCollectionDriver(Protocol):
    def open_alliance(
        self,
        request: AllianceResourceCollectionRequest,
        character: Character,
        policy: AllianceResourceCollectionPolicy,
    ) -> ResourceGatheringActionResult:
        ...

    def open_alliance_resources(
        self,
        request: AllianceResourceCollectionRequest,
        character: Character,
        policy: AllianceResourceCollectionPolicy,
    ...
    def claim_alliance_resource(
        self,
        request: AllianceResourceCollectionRequest,
        character: Character,
        observation: AllianceResourceObservation,
        policy: AllianceResourceCollectionPolicy,
    ...
    def close_reward_overlay(
        ...
        character: Character,
        observation: AllianceResourceObservation,
        claim_result: AllianceResourceClaimResult,
        policy: AllianceResourceCollectionPolicy,
    ) -> AllianceResourceClaimResult:
        ...

    def verify_alliance_resource_claimed(
        self,
        request: AllianceResourceCollectionRequest,
        character: Character,
        observation: AllianceResourceObservation,
...
@dataclass
class _AllianceResourceState:
    request: AllianceResourceCollectionRequest
    character: Character | None = None
    policy: AllianceResourceCollectionPolicy | None = None
    claim_queue: list[AllianceResourceObservation] = field(default_factory=list)
    ignored_resources: list[dict[str, object]] = field(default_factory=list)
    scanned_resources: list[dict[str, object]] = field(default_factory=list)
    claim_attempts: list[dict[str, object]] = field(default_factory=list)
    overlay_attempts: list[dict[str, object]] = field(default_factory=list)
...
class AllianceResourceCollectionWorkflow:
    def __init__(
        self,
        *,
        characters: CharacterRepository,
        driver: AllianceResourceCollectionDriver,
        account_precondition: AllianceResourceAccountPrecondition | None = None,
        character_precondition: AllianceResourceCharacterPrecondition | None = None,
        ...
        config: AllianceResourceCollectionConfig | None = None,
    ) -> None:
        ...
        self.config = config or AllianceResourceCollectionConfig()
        self._states: dict[str, _AllianceResourceState] = {}

    @property
    def workflow_states(self) -> tuple[str, ...]:
        return ALLIANCE_RESOURCE_COLLECTION_STATES
    ...
    def execute(
        ...
        *,
        ...
        token = uuid4().hex
        state = _AllianceResourceState(request=request)
        ...
        try:
            context = WorkflowExecutionContext(
                ...
                budget=StepBudget(max_steps=len(ALLIANCE_RESOURCE_COLLECTION_STATES) + 12),
                persistence=persistence,
                job_id=request.job_id,
                run_key=request.run_key or f"alliance-resource-collection:{request.instance_id}:{uuid4().hex}",
                run_attempt=request.run_attempt,
                metadata={"alliance_resource_collection_run_id": token},
            )
    ...
    def _engine(self) -> WorkflowEngine:
        ...
        for state in ALLIANCE_RESOURCE_COLLECTION_STATES:
            registry.register(f"alliance_resource_collection.{state}", self._handler_for(state))
        registry.freeze()
    ...
    def _definition(self):
        ...
        retry_limits = {
            ...
            "open_alliance_resources": self.config.navigation_retry_limit,
            "scan_claimable_resources": self.config.scan_retry_limit,
            "claim_resource": self.config.action_retry_limit,
        }
        return WorkflowDefinitionSpec(
            workflow_key=ALLIANCE_RESOURCE_COLLECTION_WORKFLOW_KEY,
            name="Collect Alliance Resources",
            steps=[
                WorkflowStepSpec(
                    step_key=state,
                    action_type=f"alliance_resource_collection.{state}",
    ...
    def _validate_input(
        ...
        return _step_result(
            ...
            data={
                ...
                "template_keys": list(ALLIANCE_RESOURCE_COLLECTION_TEMPLATE_KEYS),
            },
        )
    ...
    def _open_alliance_resources(
        ...
        return self._action_to_step(
            step,
            state,
            self.driver.open_alliance_resources(state.request, _require_character(state), _require_policy(state)),
        )

    def _scan_claimable_resources(
        self,
        step: WorkflowStepSpec,
        state: _AllianceResourceState,
        _context: WorkflowExecutionContext,
    ) -> WorkflowStepResult:
        scan = self.driver.scan_claimable_resources(state.request, _require_character(state), _require_policy(state))
        ...
        if status == AllianceResourceScanStatus.VERIFICATION_REQUIRED:
            return state.stop(
                step.step_key,
                WorkflowOutcome.FATAL_FAILURE,
        ...
        if status == AllianceResourceScanStatus.UNKNOWN:
            return state.stop(
                step.step_key,
                WorkflowOutcome.BLOCKED,
                scan.message or "Alliance resource claimable state could not be determined.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json()},
            )
        claimable = _claimable_observations(scan.observations, _require_policy(state))
        if not claimable:
            return state.stop(
                step.step_key,
                WorkflowOutcome.SKIPPED,
                scan.message or "No claimable alliance resources are available.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json(), "skipped_reason": "no_claimable_resources"},
            )
        state.claim_queue = claimable
        return _step_result(
            step.step_key,
            WorkflowOutcome.SUCCESS,
            data={
                "scan": scan.to_json(),
                "claimable_resources": [item.to_json() for item in state.claim_queue],
                "ignored_resources": state.ignored_resources,
            },
            screenshot_path=scan.screenshot_path,
        )
    ...
    def _claim_resource(
        ...
        for observation in state.claim_queue:
            context.cancellation_token.throw_if_cancelled()
            claim_result = self.driver.claim_alliance_resource(state.request, character, observation, policy)
            if claim_result.screenshot_path:
                state.screenshot_path = claim_result.screenshot_path
            self._record_claim(state, observation, claim_result)
            if not claim_result.success:
                return self._claim_failure(
                    step,
                    state,
                    claim_result,
                    fallback_message=f"Alliance {observation.normalized_resource_key()} resource claim failed.",
                )
            overlay_step = self._close_reward_overlay_action(step, state, observation, claim_result)
            if overlay_step.outcome != WorkflowOutcome.SUCCESS:
                return overlay_step
            verify_step = self._verify_claim_state_action(step, state, observation, claim_result)
    ...
    def _close_reward_overlay(
        self,
        step: WorkflowStepSpec,
        _state: _AllianceResourceState,
        _context: WorkflowExecutionContext,
    ) -> WorkflowStepResult:
        return _step_result(step.step_key, WorkflowOutcome.SKIPPED, data={"handled_during": "claim_resource"})
    ...
    def _close_reward_overlay_action(
        self,
        step: WorkflowStepSpec,
        state: _AllianceResourceState,
        observation: AllianceResourceObservation,
        ...
        if not claim_result.reward_overlay_present:
            state.overlay_attempts.append(
                {
                    "resource_key": observation.normalized_resource_key(),
                    "resource_id": observation.resource_id,
                    "overlay_present": False,
                    "closed": False,
                }
            )
            return _step_result(step.step_key, WorkflowOutcome.SUCCESS, data={"overlay_present": False})
        ...
        if not close.success:
            return self._claim_failure(step, state, close, fallback_message="Alliance reward overlay could not be closed.")
    ...
    def _verify_claim_state_action(
        ...
        verify = self.driver.verify_alliance_resource_claimed(
            state.request,
            _require_character(state),
            observation,
            _require_policy(state),
        ...
        if not _claim_postcondition_verified(verify):
            failure = replace(
                verify,
                message=(
                    f"Alliance {observation.normalized_resource_key()} claimable state remained after claim. "
                    f"{verify.message}"
                ).strip(),
            )
            return self._claim_failure(
                ...
                state,
                failure,
                fallback_message=f"Alliance {observation.normalized_resource_key()} claim postcondition was not verified.",
            )
        state.claimed_count += _claimed_count(observation, claim_result, verify)
        return _step_result(step.step_key, WorkflowOutcome.SUCCESS, data=verify.to_json(), screenshot_path=verify.screenshot_path)

    def _complete(self, step: WorkflowStepSpec, state: _AllianceResourceState) -> WorkflowStepResult:
        if state.stopped:
            return _step_result(
                step.step_key,
                WorkflowOutcome.SKIPPED,
    ...
    def _skipped(self, step: WorkflowStepSpec, state: _AllianceResourceState) -> WorkflowStepResult:
    ...
    def _failed(self, step: WorkflowStepSpec, state: _AllianceResourceState) -> WorkflowStepResult:
        if not state.failed:
            return _step_result(step.step_key, WorkflowOutcome.SKIPPED)
    ...
    def _action_to_step(
        ...
        if action.retryable:
            return _step_result(
                ...
                action.message or "Alliance resource collection action failed.",
                data=action.data,
                screenshot_path=action.screenshot_path,
            )
        return state.stop(
    ...
    def _claim_failure(
        ...
        step: WorkflowStepSpec,
        state: _AllianceResourceState,
        result: AllianceResourceClaimResult,
        *,
    ...
    def _record_scan(
        self,
        state: _AllianceResourceState,
        scan: AllianceResourceScan,
    ) -> None:
        state.scanned_resources = [item.to_json() for item in scan.observations]
        policy = _require_policy(state)
        state.ignored_resources = [
            {
                **item.to_json(),
    ...
    def _collection_payload(self, state: _AllianceResourceState) -> dict[str, object]:
        return {
            "claimed_count": state.claimed_count,
            "scanned_resources": state.scanned_resources,
            "claim_attempts": state.claim_attempts,
            "overlay_attempts": state.overlay_attempts,
    ...
    def _state_from_context(self, context: WorkflowExecutionContext) -> _AllianceResourceState:
        token = str(context.metadata.get("alliance_resource_collection_run_id") or "")
        try:
            return self._states[token]
        except KeyError as exc:
            raise RuntimeError("Alliance resource collection runtime state is missing.") from exc

    def _open_incident(self, state: _AllianceResourceState) -> None:
        if self.incidents is None or not state.failed or state.incident_opened:
            return
        self.incidents.save(
            Incident(
                incident_key=f"alliance-resource-collection:{state.request.instance_id}:{uuid4().hex}",
                severity="error",
                status="open",
                title="Alliance resource collection blocked",
    ...
    def _record_engine_failure(
        ...
        state.terminal_reason = result.message or "Alliance resource collection workflow failed."
...
def _claimable_observations(
    observations: tuple[AllianceResourceObservation, ...],
    policy: AllianceResourceCollectionPolicy,
) -> list[AllianceResourceObservation]:
    selected: list[AllianceResourceObservation] = []
    seen: set[tuple[str, str]] = set()
    for observation in observations:
        if _ignore_reason(observation, policy):
            continue
        identity = (observation.normalized_resource_key(), observation.resource_id)
        if identity in seen:
            continue
        seen.add(identity)
        selected.append(observation)
        if len(selected) >= policy.max_claims_per_run:
            break
    return selected
...
def _ignore_reason(
    ...
    if observation.normalized_resource_key() not in policy.enabled_resource_keys:
        return "disabled_resource"
...
def _claim_postcondition_verified(verify: AllianceResourceClaimResult) -> bool:
...
def _claimed_count(
    observation: AllianceResourceObservation,
    claim_result: AllianceResourceClaimResult,
    verify: AllianceResourceClaimResult,
) -> int:
    return max(1, verify.claimed_count, claim_result.claimed_count, observation.claimable_count)
...
def _step_result(
    ...
    *,
    ...
    return WorkflowStepResult(
        step_key=step_key,
        action_type=f"alliance_resource_collection.{step_key}",
        outcome=outcome,
        message=message,
        data=data or {},
        screenshot_path=screenshot_path,
...
def _require_character(state: _AllianceResourceState) -> Character:
    if state.character is None:
        raise RuntimeError("Target character has not been loaded.")
    return state.character


def _require_policy(state: _AllianceResourceState) -> AllianceResourceCollectionPolicy:
    if state.policy is None:
        raise RuntimeError("Alliance resource collection policy has not been validated.")
    return state.policy
...
def _is_manual_stop(state: _AllianceResourceState) -> bool:
