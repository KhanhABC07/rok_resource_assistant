...
LOST_CANYON_WORKFLOW_KEY = "lost-canyon"
LOST_CANYON_TEMPLATE_KEYS = (
    "city.campaign.button",
    "campaign.lost_canyon.entry",
    "lost_canyon.scene",
    "lost_canyon.unavailable",
    "lost_canyon.remaining_attempts",
    "lost_canyon.opponent",
    "lost_canyon.challenge",
    "lost_canyon.skip_battle",
    "lost_canyon.result.victory",
    "lost_canyon.result.defeat",
    "lost_canyon.result.close",
)
LOST_CANYON_STATES = (
    "validate_input",
    "load_character",
...
class LostCanyonOpponentRule(StrEnum):
    FIRST_AVAILABLE = "FIRST_AVAILABLE"
    LOWEST_POWER = "LOWEST_POWER"
    HIGHEST_CONFIDENCE = "HIGHEST_CONFIDENCE"


class LostCanyonScanStatus(StrEnum):
    READY = "READY"
...
class LostCanyonBattleOutcome(StrEnum):
    VICTORY = "VICTORY"
    DEFEAT = "DEFEAT"
    UNKNOWN = "UNKNOWN"
    VERIFICATION_REQUIRED = "VERIFICATION_REQUIRED"
...
def _opponent_rule(value: LostCanyonOpponentRule | str) -> LostCanyonOpponentRule:
    if isinstance(value, LostCanyonOpponentRule):
        return value
    try:
        return LostCanyonOpponentRule(str(value).strip().upper())
    except ValueError as exc:
        valid = ", ".join(item.value for item in LostCanyonOpponentRule)
        raise ValueError(f"Invalid Lost Canyon opponent rule: {value!r}. Expected one of: {valid}.") from exc


def _scan_status(value: LostCanyonScanStatus | str) -> LostCanyonScanStatus:
    if isinstance(value, LostCanyonScanStatus):
        return value
    try:
        return LostCanyonScanStatus(str(value).strip().upper())
    except ValueError as exc:
        valid = ", ".join(item.value for item in LostCanyonScanStatus)
        raise ValueError(f"Invalid Lost Canyon scan status: {value!r}. Expected one of: {valid}.") from exc


def _battle_outcome(value: LostCanyonBattleOutcome | str) -> LostCanyonBattleOutcome:
    if isinstance(value, LostCanyonBattleOutcome):
        return value
    try:
        return LostCanyonBattleOutcome(str(value).strip().upper())
    except ValueError as exc:
        valid = ", ".join(item.value for item in LostCanyonBattleOutcome)
        raise ValueError(f"Invalid Lost Canyon battle outcome: {value!r}. Expected one of: {valid}.") from exc
...
class LostCanyonRequest:
    ...
    session_key: str = ""
    policy: LostCanyonPolicy = field(default_factory=LostCanyonPolicy)
...
@dataclass(frozen=True)
class LostCanyonStateScan:
    status: LostCanyonScanStatus | str
    remaining_attempts: int | None = None
    opponents: tuple[LostCanyonOpponent, ...] = ()
    message: str = ""
    screenshot_path: str = ""
    data: dict[str, object] = field(default_factory=dict)

    def normalized_status(self) -> LostCanyonScanStatus:
...
@dataclass(frozen=True)
class LostCanyonBattleStartResult:
    success: bool
    changed: bool = False
...
@dataclass(frozen=True)
class LostCanyonBattleReport:
    outcome: LostCanyonBattleOutcome | str
    handled: bool = True
    remaining_attempts: int | None = None
    message: str = ""
    screenshot_path: str = ""
    ...
    def normalized_outcome(self) -> LostCanyonBattleOutcome:
...
class LostCanyonDriver(Protocol):
    def open_campaign(
        self,
        request: LostCanyonRequest,
        character: Character,
        policy: LostCanyonPolicy,
    ) -> ResourceGatheringActionResult:
        ...

    def open_lost_canyon(
        self,
        request: LostCanyonRequest,
        character: Character,
        policy: LostCanyonPolicy,
...
class LostCanyonAccountPrecondition(Protocol):
    def ensure_account(self, request: LostCanyonRequest, character: Character) -> ResourceGatheringActionResult:
        ...


class LostCanyonCharacterPrecondition(Protocol):
    def ensure_character(self, request: LostCanyonRequest, character: Character) -> ResourceGatheringActionResult:
        ...
...
@dataclass
class _LostCanyonRuntimeState:
    request: LostCanyonRequest
    character: Character | None = None
    policy: LostCanyonPolicy | None = None
    initial_scan: LostCanyonStateScan | None = None
...
class LostCanyonWorkflow:
    def __init__(
        self,
        *,
        characters: object,
        driver: LostCanyonDriver,
        account_precondition: LostCanyonAccountPrecondition | None = None,
        character_precondition: LostCanyonCharacterPrecondition | None = None,
        ...
        config: LostCanyonConfig | None = None,
    ) -> None:
        ...
        self.config = config or LostCanyonConfig()
        self._states: dict[str, _LostCanyonRuntimeState] = {}

    @property
    def workflow_states(self) -> tuple[str, ...]:
        return LOST_CANYON_STATES
    ...
    def execute(
        ...
        *,
        ...
        token = uuid4().hex
        state = _LostCanyonRuntimeState(request=request)
        ...
        try:
            ...
            context = WorkflowExecutionContext(
                ...
                budget=StepBudget(max_steps=len(LOST_CANYON_STATES) + (policy_limit * 5) + 10),
                persistence=persistence,
                job_id=request.job_id,
                run_key=request.run_key or f"lost-canyon:{request.instance_id}:{uuid4().hex}",
                run_attempt=request.run_attempt,
                metadata={"lost_canyon_run_id": token},
            )
    ...
    def _engine(self) -> WorkflowEngine:
        ...
        for state in LOST_CANYON_STATES:
            registry.register(f"lost_canyon.{state}", self._handler_for(state))
        registry.freeze()
    ...
    def _definition(self):
        ...
        retry_limits = {
            ...
            "open_lost_canyon": self.config.navigation_retry_limit,
            "fight_battles": self.config.battle_retry_limit,
        }
        return WorkflowDefinitionSpec(
            workflow_key=LOST_CANYON_WORKFLOW_KEY,
            name="Fight Lost Canyon",
            steps=[
                WorkflowStepSpec(
                    step_key=state,
                    action_type=f"lost_canyon.{state}",
    ...
    def _open_lost_canyon(
        ...
        return self._action_to_step(
            step,
            state,
            self.driver.open_lost_canyon(state.request, _require_character(state), _require_policy(state)),
            fallback_message="Lost Canyon could not be opened.",
        )
    ...
    def _inspect_attempts(
        ...
        scan = self.driver.inspect_lost_canyon(state.request, _require_character(state), _require_policy(state))
        ...
        if status == LostCanyonScanStatus.NO_ATTEMPTS or (scan.remaining_attempts is not None and scan.remaining_attempts <= 0):
            return state.stop(
                step.step_key,
                WorkflowOutcome.SKIPPED,
                scan.message or "No Lost Canyon attempts remain.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json()},
            )
        if status == LostCanyonScanStatus.FEATURE_UNAVAILABLE:
            return state.stop(
                step.step_key,
                WorkflowOutcome.SKIPPED,
                scan.message or "Lost Canyon is not currently available.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json()},
            )
        if status == LostCanyonScanStatus.VERIFICATION_REQUIRED:
            return state.stop(
                step.step_key,
                WorkflowOutcome.FATAL_FAILURE,
        ...
        if status != LostCanyonScanStatus.READY:
            return state.stop(
                step.step_key,
                WorkflowOutcome.BLOCKED,
                scan.message or "Lost Canyon attempts could not be verified.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json()},
            )
    ...
    def _fight_battles(
        ...
        if battle_limit <= 0:
            return state.stop(
                step.step_key,
                WorkflowOutcome.SKIPPED,
                "Lost Canyon policy limit is zero.",
        ...
        if not available:
            return state.stop(
                step.step_key,
                WorkflowOutcome.BLOCKED,
                "No selectable Lost Canyon opponent met policy requirements.",
                screenshot_path=scan.screenshot_path,
                data={"scan": scan.to_json()},
            )
        for battle_number in range(1, battle_limit + 1):
            ...
            if not select_result.success:
                return self._battle_failure(
                    step,
                    state,
                    select_result.message or "Lost Canyon opponent could not be selected.",
            ...
            if not start_result.success or not start_result.changed:
                return self._battle_failure(
                    step,
                    state,
                    start_result.message or "Lost Canyon battle start was not verified.",
            ...
            if policy.allow_skip_battle:
                ...
                if not skip_result.success:
                    return self._battle_failure(
                        step,
                        state,
                        skip_result.message or "Lost Canyon skip battle action failed.",
                        screenshot_path=skip_result.screenshot_path,
            ...
            if outcome == LostCanyonBattleOutcome.VERIFICATION_REQUIRED:
                return state.stop(
                    step.step_key,
                    WorkflowOutcome.FATAL_FAILURE,
            ...
            if outcome == LostCanyonBattleOutcome.UNKNOWN or not report.handled:
                return state.stop(
                    step.step_key,
                    WorkflowOutcome.BLOCKED,
                    report.message or "Lost Canyon result popup could not be handled.",
                    screenshot_path=report.screenshot_path,
                    data={"battle_number": battle_number, "result": report.to_json()},
                )
            ...
            if state.latest_remaining_attempts <= 0:
                break
        return _step_result(step.step_key, WorkflowOutcome.SUCCESS, data=self._lost_payload(state), screenshot_path=state.screenshot_path)

    def _complete(self, step: WorkflowStepSpec, state: _LostCanyonRuntimeState) -> WorkflowStepResult:
        if state.stopped:
            return _step_result(
                step.step_key,
                WorkflowOutcome.SKIPPED,
        ...
        return _step_result(step.step_key, WorkflowOutcome.SUCCESS, data=self._lost_payload(state))
    ...
    def _failed(self, step: WorkflowStepSpec, state: _LostCanyonRuntimeState) -> WorkflowStepResult:
        if not state.failed:
            return _step_result(step.step_key, WorkflowOutcome.SKIPPED)
    ...
    def _action_to_step(
        ...
        *,
        fallback_message: str = "Lost Canyon action failed.",
    ) -> WorkflowStepResult:
    ...
    def _lost_payload(self, state: _LostCanyonRuntimeState) -> dict[str, object]:
        return {
    ...
    def _state_from_context(self, context: WorkflowExecutionContext) -> _LostCanyonRuntimeState:
        token = str(context.metadata.get("lost_canyon_run_id") or "")
        try:
            return self._states[token]
        except KeyError as exc:
            raise RuntimeError("Lost Canyon runtime state is missing.") from exc

    def _open_incident(self, state: _LostCanyonRuntimeState) -> None:
        if self.incidents is None or not state.failed or state.incident_opened:
            return
        self.incidents.save(
            Incident(
                incident_key=f"lost-canyon:{state.request.instance_id}:{uuid4().hex}",
                severity="error",
                status="open",
                title="Lost Canyon workflow blocked",
    ...
    def _record_engine_failure(
        ...
        state.terminal_reason = result.message or "Lost Canyon workflow failed."
...
def _step_result(
    ...
    *,
    ...
    return WorkflowStepResult(
        step_key=step_key,
        action_type=f"lost_canyon.{step_key}",
        outcome=outcome,
        message=message,
        data=data or {},
        screenshot_path=screenshot_path,
...
def _require_character(state: _LostCanyonRuntimeState) -> Character:
    if state.character is None:
        raise RuntimeError("Target character has not been loaded.")
    return state.character


def _require_policy(state: _LostCanyonRuntimeState) -> LostCanyonPolicy:
    if state.policy is None:
        raise RuntimeError("Lost Canyon policy has not been validated.")
    return state.policy


def _require_scan(state: _LostCanyonRuntimeState) -> LostCanyonStateScan:
    if state.initial_scan is None:
        raise RuntimeError("Lost Canyon attempts have not been inspected.")
...
def _is_manual_stop(state: _LostCanyonRuntimeState) -> bool:
